...
test('findFuzzyMatch returns a confidence-scored match from the AI response', async () => {
  ...
  const fakeGenai: any = {
    models: { generateContent: async () => ({
      text: JSON.stringify({ suggestedMatchId: 'item-2', confidence: 'confirmed' }),
    }) },
  };
  const result = await findFuzzyMatch(fakeGenai, db, 'alice', 'Netflix.com');
...
test('findFuzzyMatch returns null when the AI finds no plausible match', async () => {
  ...
  const fakeGenai: any = {
    models: { generateContent: async () => ({
      text: JSON.stringify({ suggestedMatchId: null, confidence: 'low' }),
    }) },
  };
  const result = await findFuzzyMatch(fakeGenai, db, 'alice', 'Some Obscure Tool');
  expect(result).toBeNull();
});
...
test('findFuzzyMatch throws on a malformed AI response (missing confidence)', async () => {
  ...
  const fakeGenai: any = {
    models: { generateContent: async () => ({
      text: JSON.stringify({ suggestedMatchId: 'item-2' }),
    }) },
  };
  await expect(findFuzzyMatch(fakeGenai, db, 'alice', 'Netflix.com')).rejects.toThrow(
...
test('findFuzzyMatch throws on a malformed AI response (invalid confidence value)', async () => {
  ...
  const fakeGenai: any = {
    models: { generateContent: async () => ({
      text: JSON.stringify({ suggestedMatchId: 'item-2', confidence: 'maybe' }),
    }) },
  };
  await expect(findFuzzyMatch(fakeGenai, db, 'alice', 'Netflix.com')).rejects.toThrow(
    'Malformed fuzzy-match response from AI'
...
test('findFuzzyMatch returns null when the AI hallucinates an id not among the candidates', async () => {
  ...
  const fakeGenai: any = {
    models: { generateContent: async () => ({
      text: JSON.stringify({ suggestedMatchId: 'item-99', confidence: 'confirmed' }),
    }) },
  };
  const result = await findFuzzyMatch(fakeGenai, db, 'alice', 'Netflix.com');
  expect(result).toBeNull();
